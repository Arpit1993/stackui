...
const DropdownVersionCoupled = (props) => {
  ...
  const imageDate = props.imageDate.map((el, idx) => {
    return {
      type: "image",
      v: props.imageDate.length - idx,
      date: new Date(el),
    };
  });
  const labelDate = props.labelDate.map((el, idx) => {
    return {
      type: "label",
      v: props.labelDate.length - idx,
      date: new Date(el),
    };
  });

  const versions = Array()
    .concat(imageDate, labelDate)
    .sort(function (a, b) {
  ...
  if (x_min == 0) {
    ...
    let version = versions.length - x_max;
  } else {
    let version = versions.length - x_min;
  }
    ...
    options.push(
      <Menu.Item>
        {({ active }) => (
          <Tooltip
            placement="right"
            title={`${versions[x].type} version ${versions[x].v}`}
          >
            <button
              ...
              onClick={() => {
                if (x < versions.length - 1) {
                  for (let j = versions.length; j > x; j--) {
                    const y = j - 1;
                    versions[y].type == "image"
                      ? handleClick(versions[y].v, props.setD)
                      : handleClick(versions[y].v, props.setL);
                  }
                } else if (versions.length == 1) {
                  versions[0].type == "image"
                    ? handleClick(versions[0].v, props.setD)
                    : null;
                }
              }}
            >
  ...
  return (
    <Menu as="div" className="z-50 relative inline-block text-left">
      <div>{renderButton(version, versions.length)}</div>
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1 h-[150px] overflow-scroll">{options}</div>
        </Menu.Items>
      </Transition>
    </Menu>
  );
