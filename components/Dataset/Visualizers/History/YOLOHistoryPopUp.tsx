...
const YOLOHistoryPopUp = (props) => {
    ...
    <div className="text-sm fixed z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-[0.5px] border-gray-500 rounded-lg bg-white dark:bg-gray-900 w-[1100px]  h-[700px]">
        ...
        <ul className="text-xs h-1/3 font-body rounded-lg border 
                text-gray-900 bg-white border-gray-200
                  dark:bg-gray-900 dark:border-gray-600 dark:text-white">
            {changesImages.map((data, index) => <ItemFileVersion noClick={index == 0 && page == 0} key={'IFV'.concat(index.toString())} keyId={props.keyId} version={data.version} date={data.date} commit={data.commit}/>)}
        </ul>
        ...
        <ul className="text-xs h-1/3 font-body rounded-lg border 
                text-gray-900 bg-white border-gray-200
                  dark:bg-gray-900 dark:border-gray-600 dark:text-white">
            {changesLabels.map((data, index) => <ItemFileVersion noClick={index == 0 && page == 0} key={'IFV'.concat(index.toString())} keyId={labelname} version={data.version} date={data.date} commit={data.commit}/>)}
        </ul>
