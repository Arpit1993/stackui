...
function generateButtons(page, handleClick) {
  ...
  listofButtons.push(
    <div key="gs" className="flex">
      <button
        className=" bg-gray-200 flex flex-col justify-center rounded-full h-[25px] w-[25px] p-2 shadow-sm dark: text-black hover:bg-gray-300"
        onClick={() => handleClick(page, Math.max(page - 1, 0))}
      >
        {"<"}
      </button>
    </div>
  ...
  listofButtons.push(<div key="bgs">Page {page + 1}</div>);
...
const YOLOHistoryPopUp = (props) => {
    ...
    if (props.popup) {
      const getChanges = async (page) => {
        const res = await fetch(
          "http://localhost:8000/key_versions?key="
            .concat(props.keyId)
            .concat("&l=3&page=")
            .concat(page)
        );
        ...
        const res2 = await fetch(
          "http://localhost:8000/label_versions?key="
            .concat(props.keyId)
            .concat("&l=3&page=")
            .concat(page)
        );
        ...
        if (!(Object.keys(data2).length == 0)) {
          ...
          setMaxPages(
            Math.max(data.len / max_commits, data2.len / max_commits)
          );
        }
      };
      getChanges(page);
  ...
  let buttons = generateButtons(page, handleClick);
  ...
  if (props.popup == 0) {
    ...
    return (
      <div className="text-sm fixed z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-[0.5px] border-gray-500 rounded-lg bg-white dark:bg-gray-900 w-[1100px]  h-[700px]">
        ...
        <div className="text-center text-lg">{"Image Versions"}</div>

        <ul
          className="text-xs h-1/3 font-body rounded-lg border 
                text-gray-900 bg-white border-gray-200
                  dark:bg-gray-900 dark:border-gray-600 dark:text-white"
        >
          {changesImages.map((data, index) => (
            <ItemFileVersion
              noClick={index == 0 && page == 0}
              key={"IFV".concat(index.toString())}
              keyId={props.keyId}
        ...
        <div className="text-center text-lg">{"Label Versions"}</div>
        ...
        <div className="flex justify-evenly mt-5">{buttons}</div>
      </div>
    );
  }
};
