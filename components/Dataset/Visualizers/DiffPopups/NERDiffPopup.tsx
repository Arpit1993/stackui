...
const fetchData = async (keyId, version, setD) => {
  const text = await fetch(
    `http://localhost:8000/get_text?key=${keyId}&version=${version}`
  ).then((res) => res.json());
      ...
      for (let i = 0; i < order.length; i++) {
        ...
        let child: any = [
          <span
            ...
            className="w-fit text-justify h-min text-base cursor-text"
          >
            {" "}
        ...
        for (let j = 0; j < entities_per_index[start].length; j++) {
          ...
          child = [
            <button
              ...
              className="w-max text-justify relative bg-white"
              style={{
