...
import QA2Viz from "./QA2Viz";

const QA2Popup = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setDataComp([<QA2Viz key={'nervz'}  admin={true} enableLRshortcut={enableLRshortcut} setKeyId={props.setKeyId} diff={false} loading={loading} label_version={'current'} keyId={props.keyId} ww={800} wh={500} ox={0} oy={0} setnewLabels={setnewLabels} setSubmit={setSubmit}/>])            
        }
        ...
        <>  
            {
                <button key={'ccb'} onClick={() => {
                    ...
                    }} className="z-[39] bg-black/10 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-screen  h-screen">
                    click to close
                </button>
            }
            <div className="text-sm z-40 dark:bg-gray-900 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-[0.5px] border-gray-500 rounded-lg bg-white w-[1100px]  h-[600px]">
                ...
                <ul className="text-xs font-body rounded-lg 
                    ...
                    <div className="flex h-[500px] ">
                        <div className="w-full relative rounded-md dark:text-black text-center flex flex-col justify-center bg-white dark:bg-gray-900">
                            {
                                dataComp
                            }
                        </div>
