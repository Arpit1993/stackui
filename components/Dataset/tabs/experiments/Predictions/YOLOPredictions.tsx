...
const YOLOPredictions = (props) => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:8000/get_prediction?prediction=${props.prediction.current}`
    )
      .then((res) => res.json())
      .then((res) => {
        setPredictions(res["prediction"][0]);
      ...
      <div className="z-[48] p-2 grid grid-cols-4 justify-items-center gap-2 w-full h-full">
        {Object.keys(predictions).length > 0 && predictions
          ? Object.keys(predictions)
              .filter((v, i) => i >= page * 16 && i < (page + 1) * 16)
              .map((res, idx) => {
                return (
                  <PredThumbnail
                    keyIdx={idx}
...
const PredThumbnail = (props) => {
  ...
  useEffect(() => {
    ...
    setWaiting(true);
    fetch(
      `http://localhost:8000/get_thumbnail?file=${props.keys[props.keyIdx]}`,
      { signal }
    )
      .then((res: any) => res.body.getReader())
      .then(
        (reader) =>
          new ReadableStream({
            start(controller) {
  ...
  return (
    <>
      ...
      <button
        ...
        className={`justify-center flex flex-col z-10 h-[${height}px] w-[${width}px] bg-gray-50 dark:bg-gray-900 hover:bg-gray-100 hover:dark:bg-black border-[0.5px] border-gray-400 text-left text-xs`}
        key={`${props.keys[props.keyIdx].toString()}defg`}
      >
        {waiting ? (
          <div className="relative h-max flex justify-center">
            <div className="absolute top-1/2 left-1/2 animate-pulse transform -translate-x-1/2 -translate-y-1/2 flex flex-col bg-slate-400 w-full h-full"></div>
            <Image
          ...
          <Image
            className={
              props.thumbnailView ? "z-20 w-full justify-center" : "z-20"
            }
...
const ComparePrediction = (props) => {
  ...
  useEffect(() => {
    fetch(
      "http://localhost:8000/pull_file_api?file=".concat(
        props.keys[props.keyIdx]
      )
    )
      .then((res: any) => res.body.getReader())
      .then(
      ...
      .then(() => {
        fetch(
          `http://localhost:8000/get_labels?filename=${
            props.keys[props.keyIdx]
          }`
        )
  ...
  if (props.labels[props.keys[props.keyIdx]].length > 0) {
    for (let i = 0; i < props.labels[props.keys[props.keyIdx]].length; i++) {
      const w = props.labels[props.keys[props.keyIdx]][i][3] * width;
      const h = props.labels[props.keys[props.keyIdx]][i][4] * height;

      const x =
        (500 - width) / 2 +
        props.labels[props.keys[props.keyIdx]][i][1] * width -
        w / 2;
      const y =
        (500 - height) / 2 +
        props.labels[props.keys[props.keyIdx]][i][2] * height -
        h / 2;
      ...
      rect.push({
        ...
        h: Math.floor(h),
        w: Math.floor(w),
        class: props.labels[props.keys[props.keyIdx]][i][0],
        new: newRef,
      });
    }
  }

  let rectGT: Array<any> = [];
        ...
        <div className="flex flex-col h-full">
          <div className="w-full h-[50px] flex items-center justify-between p-2">
            <button
              ...
              className="bg-red-500 w-4 h-4 hover:bg-red-300 rounded-full"
            ></button>
          ...
          <div className="w-full px-5 h-fit flex gap-2 justify-end">
            <button
              onClick={() => {
                fetch(
                  `http://localhost:8000/add_tag?file=${
                    props.keys[props.keyIdx]
                  }&tag=${"prediction: false positive"}`
                );
                props.setModal(false);
              }}
              type="button"
