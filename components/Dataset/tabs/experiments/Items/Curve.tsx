...
const Curve = (props) => {
  ...
  useEffect(() => {
    if (time.current) {
      time.current = false;
      fetch(`http://localhost:8000/get_logs_experiment?log=${props.logs}`)
        .then((res) => res.json())
        .then((res) => {
          setLog(res);
        });
    ...
    let ctx = document
      .getElementById(`curve-chart for log ${props.date}`)
      .getContext("2d");
      ...
      <div className="flex justify-center">
        {log && schema ? (
          schema.value == "yolo" ? (
            <YOLOMetadata
              key={"metadata"}
              mtdt={log[log.length - 1]["metadata"]}
              date={props.date}
            />
          ) : schema.value.includes("ner") ? (
            <NERMetadata
              key={"metadata"}
              mtdt={log[log.length - 1]["metadata"]}
              date={props.date}
            />
          ) : schema.value.includes("seq") ? (
            <Seq2SeqMetadata
              key={"metadata"}
              mtdt={log[log.length - 1]["metadata"]}
              date={props.date}
            />
          ) : schema.value.includes("qa") ? (
            <QAMetadata
              key={"metadata"}
              mtdt={log[log.length - 1]["metadata"]}
              date={props.date}
