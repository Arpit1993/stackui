...
const Experiments = (props) => {
    ...
    return (
        <>
            ...
            <div className="fixed overflow-scroll p-4 z-[40] left-0 bottom-0 w-[80%] h-[80%] bg-white dark:bg-black">
...
const Project = (props) => {
    ...
    const deleteRun = (run) => {
        ...
        fetch(`http://localhost:8000/remove_log_experiment`,{
                ...
                body: data}).then(() => {
            fetch(`http://localhost:8000/get_logs_list?project=${props.project}`).then((res) => res.json()).then((res) => {setRuns(res.reverse()); props.setPopup(false)})
        })
    }
    ...
    return (
        <div className="fixed overflow-scroll z-[50] p-4 left-0 bottom-0 w-[80%] h-[80%] bg-white dark:bg-black">
            <div className="w-full flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                    Runs
                </h3>
            ...
            <div className="z-[46] p-2 grid grid-cols-1 justify-items-center gap-2 w-full h-full">
                {
                    ...
                    runs.map(
                        (run) => {
                            return <div key={`run from ${run['date']}`} className="z-[100] block h-fit w-full p-6 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
