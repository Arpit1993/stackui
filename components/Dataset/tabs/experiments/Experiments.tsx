...
import ImageThumbnail from "../../explorer/FileExplorer/Items/ImageThumbnail";
import YOLOPredictions from "./Predictions/YOLOPredictions";
...
const Experiments = (props) => {
    ...
    return (
        <>
            {
                popup ? 
                <Project project={project} schema={props.schema} setPopup={setPopup}/>
                :   
...
const Project = (props) => {
    ...
    const [predictions, setPredictions]= useState<any>(false)
    const prediction = useRef('')
    const deleteRun = (run) => {
        const data = JSON.stringify({experiment: props.project, log: run})
        fetch(`http://localhost:8000/remove_log_experiment`,{
                method: 'POST',
                headers: { 
                    "Content-Type": "application/json" 
                }, 
                body: data}).then(() => {
            fetch(`http://localhost:8000/get_logs_list?project=${props.project}`).then((res) => res.json()).then((res) => {setRuns(res.reverse())})
        })
    }
    ...
    return (
        <div className="w-full h-full bg-white dark:bg-black">
            {
                (predictions && prediction.current != '' && (props.schema == 'yolo' || props.schema == 'labelbox')) ?
                <YOLOPredictions prediction={prediction} setPopup={setPredictions}/>
                : 
                <>
                    <div className="w-full flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                        <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                    ...
                    <div className="z-[46] p-2 grid grid-cols-1 justify-items-center gap-2 w-full h-full">
                        {
                            ...
                            runs.map(
                                (run) => {
                                    return  <div key={`run from ${run['date']}`} className="z-[100] block h-fit w-full p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
                                        <div className="flex justify-between">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => {prediction.current = run['prediction'][0]; setPredictions(true)}} className="text-gray-400 border border-gray-200 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="defaultModal">
                                                    <SsidChartIcon className="w-5 h-5"/>
                                                </button>  
                                                <button onClick={() => {deleteRun(run['logs'])}} className="text-gray-400 border border-gray-200 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="defaultModal">
