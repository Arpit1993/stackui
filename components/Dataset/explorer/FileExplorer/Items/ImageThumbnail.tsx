...
const ImageThumbnail = (props) => {
  ...
  useEffect(() => {
    ...
    setWaiting(true);
    fetch(`http://localhost:8000/get_thumbnail?file=${props.file["name"]}`, {
      signal,
    })
  ...
  return (
    <div
      ...
      className={`relative h-[${height}px] w-[${width}px]`}
    >
      {popup ? (
        <FileTagPopup
        ...
        <Tooltip title={`${msg}`} placement="right">
          {anomaly ? (
            <button
              ...
              className="absolute mt-1 ml-1 z-20"
            ...
            <button
              key={`tags-${props.file["name"]}`}
              className="absolute border z-20 ml-1 mt-1 w-[15px] h-4 bg-red-500 rounded-full hover:bg-red-700"
              onClick={() => setPopup(true)}
            ></button>
          )}
        </Tooltip>
      ...
      <div className={`w-full h-full flex justify-center z-10`}>
        <button
          className={`justify-center flex flex-col z-10 h-full w-fit bg-gray-50 dark:bg-gray-900 hover:bg-gray-100 hover:dark:bg-black border-[0.5px] border-gray-400 text-left text-xs`}
        ...
        >
          {
            <button
              className={
                props.selected[props.index]
                  ? `absolute z-[21] justify-center flex flex-col rounded-lg h-full w-full bg-blue-500/30 hover:bg-blue-500/50`
                  : `absolute  z-[21] justify-center flex flex-col rounded-lg h-full w-full hover:bg-white/20`
              }
