...
const FileExplorer = (props) => {
  ...
  let idx_min: number = 0;
  let idx_max: number = 0;

  const view_label = props.view ? "Grid View" : "List view";

  let container_var: Array<any> = [];
      ...
      if (props.shortcuts.current == true) {
        if (event.shiftKey) {
          if (event.key == "_") {
            ...
            props.setMaxView(
              Math.floor(
                Math.min(Math.pow(Math.sqrt(props.max_view) + 1, 2), 36)
              )
            );
            props.setPage(0);
          } else if (event.key == "+") {
            ...
            props.setMaxView(
              Math.floor(
                Math.max(Math.pow(Math.sqrt(props.max_view) - 1, 2), 1)
              )
            );
            props.setPage(0);
          }
          ...
          if (event.key == "ArrowLeft") {
            ...
            if (sol == -1) {
              ...
              if (pointer < props.files.length - 1) {
                for (let i = 0; i < 6; i++) {
                  surr.splice(
                    Math.min(pointer + 1 + i, props.files.length - 1),
                    1,
                    !surr[Math.min(pointer + 1 + i, props.files.length - 1)]
                  );
                }
                setSelected(surr);
              }
        ...
        if (event.ctrlKey) {
          if (event.key == "a") {
            event.preventDefault();
            setSelected(
              Array(props.files.length).fill(selected.includes(false))
            );
  ...
  if (props.view) {
    ...
    container_var.push(
      <div key={"fctnr1"}>
        ...
        <div className="grid grid-rows-20 gap-1">
          {files
            .filter((item, index) => index < idx_max && index >= idx_min)
            .map((file, index) => (
              <button
                className="w-full text-left"
                key={`${index.toString()}abc`}
                onClick={() => handleObjectClick(file["name"])}
              >
    ...
    let class_var: string | null = null;
