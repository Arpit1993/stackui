    ...
    if (varFilter[cl]) {
      var_buttons.push(
        <ul key={`abc ${cl} ${name}`} className="w-full flex px-1 mb-1 mt-1">
          ...
          >
            {`${Object.keys(labelMap).length ? labelMap[cl]["label"] : cl} (${
              n_var[cl]
            })`}
          </button>
        </ul>
...
const YOLOFilterPopup = (props) => {
  ...
  const handleApplyFilter = async () => {
    ...
    for (let i = 0; i < Object.keys(classesFilter).length; i++) {
    ...
    for (let i = 0; i < Object.keys(resFilter).length; i++) {
    ...
    if (semanticSearch.length > 0) {
      filters[idx] = {
        semantic_search: { text: semanticSearch, threshold: slideThreshold },
      };
      idx += 1;
    }
    ...
    if (sliderClasses[0] > 0 || sliderClasses[1] < 100) {
      filters[idx] = {
        num_classes: [
          Math.floor((numClasses * sliderClasses[0]) / 100),
          Math.floor((numClasses * sliderClasses[1]) / 100),
        ],
      };
      idx += 1;
    }
  ...
  const handleResetFilter = async () => {
    ...
    await fetch("http://localhost:8000/reset_filters/")
      ...
      .then(() => {
        ...
        for (let i = 0; i < Object.keys(rFilter).length; i++) {
          rFilter[Object.keys(rFilter)[i]] = false;
        }

        const cFilter = classesFilter;
        for (let i = 0; i < Object.keys(cFilter).length; i++) {
  ...
  let classes_buttons: Array<any> = getbuttons(
    classes,
    classesFilter,
    setClassFilter,
    nullStr,
  ...
  let res_buttons: Array<any> = getbuttons(
    resolutions,
    resFilter,
    setResFilter,
    nullStr,
  ...
  return (
    <>
      ...
      <div
        key={"flterpp"}
        ref={ref}
        className="transform -translate-x-1/2 left-1/2 bg-white absolute flex flex-col items-center z-40 top-16 rounded-lg dark:bg-gray-900 w-[85%] h-[330px] border-[0.5px] border-gray-500"
      >
        <div className="w-full justify-between flex h-8">
          ...
          <div className="px-2">
            <button
              ...
              className="text-xs text-center text-white font-base w-[120px] h-[20px] bg-blue-600 hover:bg-blue-800 rounded-full"
            >
              structured query
            </button>
          </div>
        ...
        <div className="w-[90%] mt-2 mb-2 items-center">
          <div className="text-sm">Semantic search</div>
          <div className="flex items-center gap-2">
            <div className="dark:text-white w-[80%]">
              <input
                onChange={(e) => {
                  setSemanticSearch(e.target.value);
                }}
                placeholder="AI-Powered search (detailed queries lead to better results)"
        ...
        {query ? (
          ...
          <div className="flex w-full overflow-x-scroll justify-center gap-2 p-2">
            ...
            <div>
              <div className="text-sm">Resolutions</div>
              <div className="h-[100px] w-[200px] border rounded-md shadow-inner border-gray-300">
                <div className="overflow-y-scroll h-[98px]">{res_buttons}</div>
              </div>
            </div>
            <div>
            ...
            <div className="h-[120px] w-[200px]">
              <div className="text-sm">Bounding box area</div>
              <div className="w-[200px] h-8 border rounded-md shadow-inner border-gray-300">
                {/* <div className="flex gap-1 text-xs px-1 w-[198px] text-center rounded-t-md dark:bg-gray-900 bg-gray-200">
                                </div> */}
                <div className="w-full px-5">
              ...
              <div className="mt-[15px] text-sm">Objects per image</div>
              <div className="w-[200px] h-8 border rounded-md shadow-inner border-gray-300">
                {/* <div className="flex gap-1 text-xs px-1 w-[198px] text-center rounded-t-md dark:bg-gray-900 bg-gray-200">
                                </div> */}
                <div className="w-full px-5">
