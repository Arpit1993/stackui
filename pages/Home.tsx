...
export default function Main() {
  ...
  let data_array =
    datasets.length > 0 && datasetsDict && hierarchy
      ? sortOrder
        ? [...datasets].sort(compare_fcn)
        : [...datasets].sort(compare_fcn).reverse()
      : null;
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <article className="flex w-full">
        <aside className="z-10 w-[40%] h-full flex flex-col items-center gap-20 justify-center">
          <div className="p-8 text-5xl flex flex-col justify-center items-center font-bold h-max text-center">
            ...
            <h1 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl md:text-6xl">
              <span className="animate-pulse block xl:inline bg-gradient-to-r bg-clip-text text-transparent from-blue-400 to-blue-800 via-blue-400 animate-text">
                Supercharge{" "}
              </span>{" "}
          ...
          <div className="px-20 flex h-max justify-center gap-2">
            <Link
              href="https://stackai.gitbook.io/stack-beta-release/"
              passHref
            >
              <button className="h-max block p-6 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                ...
                <p className="font-normal text-gray-700 dark:text-gray-400">
                  Learn more about how to set up and use Stack.
                </p>
              </button>
            </Link>
          </div>
        </aside>
        <aside className="h-screen overflow-scroll w-[60%]">
            ...
            <div className="w-[90%] h-full">
              <div className="w-full flex justify-between">
                ...
                <div className="flex w-[40%] justify-end">
                  ...
                  >
                    {`Order by ${
                      sortMode == 0 ? "date" : sortMode == 1 ? "name" : "URI"
                    }`}
                  </button>
                  <button
                    onClick={() => {
                    ...
                    className="flex h-fit w-36 items-center gap-2 py-2.5 px-5 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
                  >
              ...
              <div className="h-full mt-2 border border-gray-300 dark:border-gray-700 rounded-md overflow-y-scroll flex flex-col w-full overflow-clip">
                {data_array
                  ? data_array
                      .filter((dataset) =>
                        hierarchy[dataset.storage]
                          ? hierarchy[dataset.storage]["parent"] == ""
                          : true
                      )
                      .map((dataset) =>
                        dataset && dataset.name ? (
                          <>
                            ...
                            {hierarchy && hierarchy[dataset.storage]
                              ? hierarchy[dataset.storage]["children"].length >
                                0
                                ? hierarchy[dataset.storage]["children"].map(
                                    (dataset1) =>
                                      datasetsDict && datasetsDict[dataset1] ? (
                                        <BranchComponent
                                          datasets={datasets}
                                          key={`dataset${datasetsDict[dataset1].name}`}
                                          dataset={datasetsDict[dataset1]}
                                          datasetsDict={datasetsDict}
                                          hierarchy={hierarchy}
                                          setLoading={setLoading}
                                        />
                                      ) : null
